...
async function dbActors() {
  ...
  try {
    const publicBootstrap = await modules.bootstrapForUser("");
    const plan = targetPlan(publicBootstrap.plans, query);
    const [rows] = await modules.pool.query(
      `SELECT u.id, u.display_name, 'owner' AS role
         FROM plans p JOIN users u ON u.id = p.owner_user_id
        WHERE p.id = ? AND p.deleted_at IS NULL
       UNION
       SELECT u.id, u.display_name, pag.role
         FROM plan_access_grants pag JOIN users u ON u.id = pag.user_id
        WHERE pag.plan_id = ? AND pag.status = 'active' AND pag.role IN ('owner', 'editor')
...
async function dbExport() {
  ...
  try {
    ...
    const plan = targetPlan(bootstrap.plans, query);
    assertEditorAccess(bootstrap, plan.id, actor);
    output(snapshot(bootstrap, plan, "database", actor), option("--out"));
  } finally {
    await modules.closePool();
  }
}
...
function countSummary(content) {
  return Object.fromEntries(Object.entries(content).map(([key, rows]) => [key, rows.length]));
}
...
async function dbApply() {
  ...
  try {
    ...
    const currentPlan = targetPlan(currentBootstrap.plans, proposed.plan.id);
    ...
    console.error(JSON.stringify({
      ...
      before: countSummary(current.content),
      after: countSummary(proposed.content),
      commit: flag("--commit"),
    }, null, 2));
