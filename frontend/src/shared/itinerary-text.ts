...
function itemLine(item: ItineraryItem, nameOf?: (id: string) => string): string {
  ...
  // 一部メンバーだけの予定（途中合流の移動など）は誰の予定かを注記する。
  if (Array.isArray(item.members) && item.members.length) {
    const names = nameOf ? item.members.map(nameOf).filter(Boolean) : [];
    line += `〔${names.length ? names.join("・") : "一部メンバー"}のみ〕`;
  }
  return line.trim();
}
...
export function buildItineraryShareText(
  trip: Partial<TripInfo> | undefined,
  days: ShareDay[],
