...
function toMeta(row: db.PlanRow): PlanMeta {
  ...
  return {
    ...
    members: ids.map((id) => db.planMemberName(row.id, id)).filter(Boolean).join("、"),
    memberIds: ids,
    note: row.note || "",
    cover: row.cover_url || "",
...
export function getData(slug: string): LocalPlanData | null {
  ...
  return {
    trip: {
      ...
      members: memberIdsOf(row.id).map((id) => db.planMemberName(row.id, id)).filter(Boolean).join("、"),
      note: row.note || "",
      cover: row.cover_url || "",
    } as TripInfo,
    itinerary: db.itinerary().filter((i) => i.plan_id === row.id).map((it) => ({
      ...
      publicTrackKeys: it.public_track_keys?.length ? it.public_track_keys : undefined,
    ...
    candidates: db.candidates().filter((c) => c.plan_id === row.id).map((c) => ({
      ...
      title: c.title,
      place: c.place || "",
      proposer: db.planMemberName(row.id, c.proposed_by_id) || undefined,
      ...
      votes: votes.filter((v) => v.candidate_id === c.id).map((v) => db.planMemberName(row.id, v.user_id)).filter(Boolean),
      createdAt: "",
